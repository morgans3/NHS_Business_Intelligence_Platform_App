...
export class DashboardsTableComponent implements OnInit {
    ...
    ngOnInit() {
        this.dynApiService.getDashboards().subscribe((orgs: any) => {
    ...
    addEdit(dashboard = null) {
        import("../dashboard/dashboard.modal").then((c) => {
            let dialog = this.dialog.open(c.DashboardModalComponent, {
            ...
            dialog.afterClosed().subscribe((data) => {
                if (data) {
                    if (dashboard !== null) {
                        ...
                        let filteredListIndex = this.dashboards.filtered.findIndex((listedDash) => listedDash.code == dashboard.code);
    ...
    delete(dashboard) {
        this.notificationService.question("Are you sure you want to delete this dashboard?").then((confirmed) => {
            if (confirmed == true) {
                this.dynApiService.archiveDashboard(dashboard).subscribe((res) => {
                    //Notify success
