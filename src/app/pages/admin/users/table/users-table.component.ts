...
export class UsersTableComponent implements OnInit {
    ...
    getUsers(startItem = null) {
        // Set page key?
        if (startItem) {
            this.filters.pageKey = JSON.stringify(startItem);
        }
    ...
    delete(item) {
        this.notificationService.question("Are you sure you want to delete this user?").then((confirmed) => {
            if (confirmed === true) {
                this.apiService.deleteUser(item.username, item.organisation).subscribe(() => {
                    // Notify success
                    this.notificationService.success("User has been removed successfully!");

                    // Change item at index
                    this.users.all.splice(
                        this.users.all.findIndex(
                            (listedItem) => listedItem.username === item.username && listedItem.organisation === item.organisation
                        ),
                        1
                    );
