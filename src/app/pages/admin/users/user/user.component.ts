...
export class UserComponent implements OnInit {
    ...
    ngOnInit() {
        this.activatedRoute.params.subscribe((params) => {
            //Get user's details
            this.userGroupService.getUserProfileByUsername(params.username).subscribe((user) => {
                this.user.profile = user;
            });  

            //Get user's teams
            this.userGroupService.getTeamMembershipsByUsername(params.username).subscribe((teams: any) => {
    ...
    team = {
        search: (name) => {
            this.userGroupService.searchTeamsByName(name).subscribe((teams: any) => {
                this.teams = teams;
                console.log(teams);
            })
        },
