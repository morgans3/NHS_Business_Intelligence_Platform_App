    ...
    ngOnInit() {
        this.activatedRoute.params.subscribe((params) => {
            if (params.id && params.id !== 'new') {
                this.authService.getCapabilityById(params.id).subscribe((capability: any) => {
                    //Set values
                    this.capability.patchValue(Object.assign(capability, {
    ...
    save() {
        if (this.capability.valid) {
            ...
            if(this.capability.value.id) {
                method = this.authService.updateCapability(this.capability.value);
            } else {
                method = this.authService.createCapability(this.capability.value);
            }
