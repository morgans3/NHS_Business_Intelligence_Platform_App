...
export class GSIComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        ...
        this.policeService.getCategories().subscribe((res: { url: string; name: string }[]) => {
    ...
    applyPoints(event) {
        ...
        switch (event.name) {
            case "Crimes":
                if (event.value) {
                    if (event.target === "secondary") {
                        this.addCrimeMarkers(bounds, map, this.pinGroupSecondary, this.MapTwoZoom);
                    } else {
                        this.addCrimeMarkers(bounds, map, this.pinGroupPrimary, this.MapOneZoom);
                    }
                }
                break;
            default:
    ...
    addCrimeMarkers(mapbounds, map, layer, zoom) {
        if (zoom < 14) {
            this.notificationService.warning("Unable to obtain crimes at this level, please zoom in.");
        } else {
            if (!layer) {
            ...
            this.policeService.getWithinBoundary(mapbounds).subscribe((res: any) => {
                if (res.length > 0) {
                    this.notificationService.info(
                        `Displaying the ` + res.length + ` crimes retrieved from Police.gov.uk within the current map boundaries`
                    );
                    this.loadedCrimes = res;
                    res.forEach((crime) => {
                        if (crime.location) {
                            if (crime.location.latitude && crime.location.longitude) {
                                const color = CrimeCategoryColors.find((x) => x.url === crime.category) || { color: "grey" };
                                L.marker(
                                    {
                    ...
                    this.notificationService.info(`No crimes found within the bounds of the current map.
    ...
    crimePopup(crime) {
        let response =
            `<strong>` + this.CrimeCategories.find((x) => x.url === crime.category).name + `</strong><p>Date: ` + crime.month + `</p>`;
    ...
    initMosaicView(map) {
        ...
        this.postCodePoly = new L.GeoJSON(this.PC15, {
            ...
            .bindTooltip((layer) => {
                const mosaic: MosaicCode = this.mosaicCodes.find((x) => x.code === layer["feature"].properties.f1);
    ...
    MosaicClickEvent(mosType) {
        ...
        this.changeMapTitle(mosType + ": " + this.mosaicCodes.find((x) => x.code === mosType).name);

        this.displayKey = false;
        this.displayTypesPrimary[this.displayTypesPrimary.findIndex((x) => x.value === "thematic")].disabled = true;
    ...
    initThematicView(map) {
        ...
        this.showgiGraph = false;
        this.showgiTable = false;
        this.displayTypesPrimary[this.displayTypesPrimary.findIndex((x) => x.value === "thematic")].disabled = false;
        this.group.patchValue({ display: "thematic" });
        this.displayTypesPrimary[this.displayTypesPrimary.findIndex((x) => x.value === "single")].disabled = true;
        this.mapTitle =
            "Thematic Map: " + this.group.value["category"] + " ⇛ " + this.group.value["topic"] + " ⇛ " + this.group.value["variable"];

        map.layers = [];

        const MosaicTypeLookupValAndRange = {};
        const MosaicTypeLookup = this.gsiData[this.group.value["category"]][this.group.value["topic"]][this.group.value["variable"]];
        ...
        for (const key3 in MosaicTypeLookup) {
            ...
            MosaicTypeLookupValAndRange[key3] = [
                ...
                (MosaicTypeLookup[key3] - variableRange[0]) / (variableRange[1] - variableRange[0]),
            ];
        }
        ...
        for (const key4 in this.postCodePoly._layers) {
            ...
            this.postCodePoly._layers[key4].feature.properties.themeVal =
                MosaicTypeLookupValAndRange[this.postCodePoly._layers[key4].feature.properties.f1];
        }
    ...
    initSingleView(mosType: any) {
        this.postCodePoly.setStyle((e) => {
            return this.styleHighlightType(e, mosType, MosaicColorCodes.find((x) => x.code === mosType.substring(0, 1)).color);
        });

        this.displayTypesPrimary[this.displayTypesPrimary.findIndex((x) => x.value === "single")].disabled = false;
    ...
    changeTopic(value: any) {
        this.variables = Object.keys(this.gsiData[this.group.value["category"]][value]);
        this.displayKey = false;
        this.thematicSelection(false);
    ...
    stylePostcodeType(feature) {
        let color = "#FFFFFF";
        color = MosaicColorCodes.find((x) => x.code === feature.properties.f1.substring(0, 1)).color;
        return {
            fillColor: color,
            weight: 0.25,
            opacity: 1,
    ...
    styleHousehold(feature) {
        let color = "#FFFFFF";
        color = MosaicColorCodes.find((x) => x.code === feature["properties"].MosaicGroup.substring(0, 1)).color;
        return {
            fillColor: color,
            radius: 4,
            weight: 0,
