...
export class MosaicKeyComponent implements OnInit, OnChanges {
    ...
    drawKey(SortedKey) {
        ...
        const linearScale = d3.scaleLinear().domain([start, end]).range([0, width]);
        ...
        if (allKeys !== undefined && allKeys["mainKey"] !== undefined) {
            ...
            this.drawBars(this.mosaicColourFromData, 2 * keyBarHeight, this.mosTypeBars, "mos", linearScale, keyBarHeight, duration);
            ...
            if (firstRun) {
                ...
                allKeys["mainKey"].select(".x").transition().duration(duration).call(xAxis);
            }

            keyToolTip = d3.select("mat-sidenav-content").append("div").attr("class", "tooltip").style("opacity", 0);
        ...
        if (allKeys !== undefined && allKeys["mainKey"] !== undefined) {
            ...
            allKeys["mainKey"].on("mouseout", function () {
                ...
                keyToolTip.transition().duration(200).style("z-index", 9999).style("opacity", 0);
            });
        }
    }
