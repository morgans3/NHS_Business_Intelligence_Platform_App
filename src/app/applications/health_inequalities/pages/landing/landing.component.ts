...
export class HiLandingComponent implements OnInit {
    ...
    setToolbarCardHeight() {
        ...
        this.toolbarMain.nativeElement.setAttribute("style", "height: " + innerCardHeight + "px");
    }
    ...
    init_svg() {
        ...
        this.plotheight = (this.plotwidth / 5) * 3;
    ...
    draw_axis() {
        ...
        this.g
            ...
            .attr("transform", "translate(" + (0.5 * this.width) / 2 + "," + (this.plotheight - 25) + ")")
            .text("SII [# per 100,000]");
        // x-axis label
        this.g
            .append("text")
            .attr("text-anchor", "middle")
            .attr("transform", "translate(" + (1.5 * this.width) / 2 + "," + (this.plotheight - 25) + ")")
            .text("SII [# per 1000]");
        // y-axis label
        this.g
            .append("text")
    ...
    one_up() {
        switch (this.level) {
            ...
            case 2:
                ...
                break;
            case 3:
                this.draw_aii = this.sub_item(this.old_aii[0].drilldown, this.a_chapter.toString(), "up");
                this.draw_mii = this.sub_item(this.old_mii[0].drilldown, this.m_chapter.toString(), "");
                break;
            case 4:
                this.draw_aii = this.sub_item(
                    this.sub_item(this.old_aii[0].drilldown, this.a_chapter.toString(), ""),
                    this.a_cat1.toString(),
                    "up"
                );
                this.draw_mii = this.sub_item(
                    this.sub_item(this.old_mii[0].drilldown, this.m_chapter.toString(), ""),
            ...
            case 5:
                this.draw_aii = this.sub_item(
                    this.sub_item(this.sub_item(this.old_aii[0].drilldown, this.a_chapter.toString(), ""), this.a_cat1.toString(), ""),
                    this.a_cat2.toString(),
                    "up"
                );
                this.draw_mii = this.sub_item(
                    this.sub_item(this.sub_item(this.old_mii[0].drilldown, this.m_chapter.toString(), ""), this.m_cat1.toString(), ""),
    ...
    init_dsr() {
        ...
        this.dsr_width = this.dsr_plotwidth - 3 * this.margin.left - this.margin.right;
        this.dsr_height = this.dsr_plotheight - this.margin.top - this.margin.bottom;
        this.dsr_g = this.dsr_svg.append("g").attr("transform", "translate(" + this.margin.left + "," + this.margin.top + ")");
    }
    // Define plot axis lengths
    init_dsr_axis(data: any[]) {
        this.dsr_x = d3Scale.scaleLinear().rangeRound([2 * this.margin.left, this.dsr_width]);
        this.dsr_y = d3Scale.scaleLinear().rangeRound([this.dsr_height, 0]);
        this.dsr_y.domain([0, Math.ceil(d3_test.max(data.map((d) => d.standard_rate)))]);
    ...
    draw_dsr_axis(rate: string) {
        ...
        // Swap y-axis label side for negative values
        this.dsr_g.selectAll(".dsr_tick").select("text").attr("x", "-9").style("text-anchor", "end");
        // Mortality x-axis
        this.dsr_g
            .append("g")
            .attr("class", "axis axis--x")
        ...
        this.dsr_g
            .append("text")
            .attr("text-anchor", "middle")
            .attr("transform", "translate(" + this.dsr_width / 2 + "," + (this.margin.bottom + this.dsr_height) + ")")
            .text("IMD Group");
        // y-axis label
        this.dsr_g
            .append("text")
