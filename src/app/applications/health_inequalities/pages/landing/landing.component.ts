...
export class HiLandingComponent implements OnInit {
  ...
  init_axis() {
    ...
    this.a_x.domain([0, Math.max.apply(Math, this.draw_aii.map(d => d.value))]);
  }
  ...
  showTooltip(datum: any, type: string) {
    ...
    if (type === "Mortality") {
      this.plot_dsr(m_dsr[0].filter(x => x.id === datum.name), rate_multiplier);
    } else {
      this.plot_dsr(a_dsr[0].filter(x => x.id === datum.name), rate_multiplier);
    };
  }
