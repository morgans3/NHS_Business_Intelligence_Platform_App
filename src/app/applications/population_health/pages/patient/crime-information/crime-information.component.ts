...
export class CrimeInformationComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.lat && this.lng) {
      this.http.get(`https://data.police.uk/api/crimes-street/all-crime?lat=${this.lat}&lng=${this.lng}`).subscribe((res: Crime[]) => {
        if (res) {
          this.crimes = res;
          this.getStats();
        }
  ...
  getStats() {
    ...
    this.crimes.reduce((previous, current, index, array) => {
      this.categoryCount[current.category] =
        (this.categoryCount[current.category] || 0) + 1;
      return previous;
    });
