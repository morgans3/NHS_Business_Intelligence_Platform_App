...
export class TheographComponent implements OnInit {
    ...
    this.focusCircle = this.hevents
      ...
      .on("click", (d, me, t1) => {
        ...
        const elemRect = document
          .getElementById("admissionDetails")
        ...
        this.div
          ...
          .style(
            "width",
            document.getElementById("admissionDetails").getBoundingClientRect()
              .width + "px"
          )
  ...
  convertDateToUS(date: string) {
    return (
      date.substr(3, 2) + "-" + date.substr(0, 2) + "-" + date.substr(6, 2)
    );
  }
  ...
  brushed() {
    ...
    this.focusBars
      ...
      .attr("width", d => {
        const fdate = new Date();
        fdate.setTime(
          new Date(this.convertDateToUS(d.EventDate)).getTime() +
            d.LengthOfStay * 1000 * 60 * 60 * 24
        );
        return (
          this.x(fdate) - this.x(new Date(this.convertDateToUS(d.EventDate)))
        );
      });
