...
import { Cohort, NiceResponse, APIService } from "diu-component-library";
...
export class InterventionAssistantComponent implements OnInit {
  ...
  no_nice_response = [
    {
      Title:
        "No Interventions or Guidance Found For This Specific Cohort, Please Try A Broader Cohort Search.",
      Abstract: "",
      EvidenceTypes: "",
      SourceUrl: ""
    }
  ...
  no_response = [
    {
      official_title:
        "No Trials Found For This Specific Cohort, Please Try A Broader Cohort Search.",
      source_id: "",
      enrollment: "",
      status: "",
      registered: "",
  ...
  intervention_types = [
    {
      description:
        "Nice Evidence Search (Including Wider Determinents of Health, e.g. smoking, employment)",
      name: "primary"
    },
  ...
  send_api() {
    switch (this.selected_type.name) {
      case "secondary":
        const cohort = this.cohort_array.find(
          x => x.cohortName === this.selected_cohort
        );
        let phase_request = "";
        ...
        this.nice_div = false;
        this.loading = true;
        this.apiService.searchClinicalTrials(cohort.cohorturl, phase_request, date_request.toString()).subscribe(res => {
          console.log(res);
          this.loading = false;
          this.get_data(res);
        });
      ...
      default:
        ...
        if (!this.specific_intervention || this.specific_intervention === "") {
          const nice_cohort = this.cohort_array.find(
            x => x.cohortName === this.selected_cohort
          );
          ...
          if (parsed_cohort["LTCs2Dimension"]) {
            ...
            this.loading = true;
            this.apiService
              .searchNICEEvidence(nice_query, 250)
              .subscribe((res: any) => {
