...
export class NeedListComponent {
  ...
  response_variable = ["Asthma", "Coronary Artery Disease", "Congestive Heart Failure", "Cancer", "COPD", "Depression", "Diabetes", "Hypertension", "Atrial Fibrillation", "Chronic Kidney Disease", "Dementia", "Epilepsy", "Hypothyroid", "Mental Health", "Learning Disability", "Osteoporosis", "Peripheral Artery Disease", "Rheumatoid Arthritis", "Stroke/TIA", "Total Medical Cost [£]", "AE Attendances", "Outpatient Appointments", "Inpatient Appointments"];
  response_variableCopy: string[] = [];
  // Predictor variable array
  predictors = ["Age", "Age Band", "Sex", "Deprivation Decile", "Number of Other Long-Term Conditions", "Average Days Between Appointments", "Smoking History", "Substance Misuse", "Psychotic Disorder", "Mosaic Label", "Asthma", "Coronary Artery Disease", "Congestive Heart Failure", "Cancer", "COPD", "Depression", "Diabetes", "Hypertension", "Atrial Fibrillation", "Chronic Kidney Disease", "Dementia", "Epilepsy", "Hypothyroid", "Mental Health", "Learning Disability", "Osteoporosis", "Peripheral Artery Disease", "Rheumatoid Arthritis", "Stroke/TIA", "Total Medical Cost [£]", "AE Attendances", "Outpatient Appointments", "Inpatient Appointments", "Palliative Care Status", "Care Home Status", "Frailty"];
  ...
  training_groups = [
    {
      name: "Blackpool CCG",
      disabled: false,
      area: ["Blackpool CCG", "Blackpool Central West - Blackpool CCG", "Central - Blackpool CCG", "North - Blackpool CCG", "Other PCN - Blackpool CCG", "South - Blackpool CCG", "WIN - Blackpool CCG"],
    },
    {
      name: "Fylde and Wyre CCG",
      disabled: false,
      area: ["Fylde and Wyre CCG", "WREN - F and W CCG", "WIN - F and W CCG", "Fleetwood - F and W CCG", "Lytham, Ansdell and St Annes - F and W CCG"],
    },
  ];
  ...
  server_error_toaster(error_status: string) {
    this.toastr.show("If The Problem Persists Then Contact A Nexus Intelligence Admin.", "Warning, " + error_status + " Error. Please Wait A Few Seconds And Try Again.", { timeOut: 3000 });
  }
  ...
  send_api() {
    ...
    if (this.use_response[0] == null) {
      ...
      } else if (this.use_response.includes("AE Attendances") || this.use_response.includes("Outpatient Appointments") || this.use_response.includes("Inpatient Appointments")) {
        this.ltcDiv = false;
  ...
  private draw_axis() {
    ...
    // x-axis
    this.g.append("g").attr("class", "axis axis--x").call(d3Axis.axisTop(this.x).ticks(10, "%")).append("text").attr("class", "axis-title").attr("transform", "rotate(-90)").attr("x", 6).attr("dx", "1em").style("text-anchor", "middle");
  }
  ...
  private draw_bars() {
    ...
    bars.on("mouseenter.something", (d, index, array) => this.mouseEnter(d, index, array, "normal")).on("mouseout.something", () => this.mouseLeave());
  }
  ...
  htmlTooltip(d: any) {
    ...
    output += Math.round(100 * (d.match_ratio - 1)) + " &plusmn; " + Math.round((100 * (d.match_higher - d.match_lower)) / 2) + "%</h5>";
    output += "		</div>";
  ...
  private draw_roc_pr_axis() {
    // y-axis
    this.roc_pr_g.append("g").attr("class", "axis axis--y").call(d3Axis.axisLeft(this.roc_pr_y)).append("text").attr("transform", "rotate(-90)").attr("y", 6).attr("dy", "0.71em").style("text-anchor", "end").text("Precision");
    ...
    this.roc_pr_g
      ...
      .on("mousemove", () => {
        ...
        this.tooltip
          ...
          .html("<p> Area Under the Curve = " + d2.auc + "</p><br> \
              <p> False Positive Rate = " + d2.false_positive + "</p><br> \
              <p> True Positive Rate = " + d2.true_positive + "</p><br> \
              <p> Cutoff = " + d2.cutoff + "</p>");
  ...
  private draw_pr_line() {
    ...
    let pr_line = this.roc_pr_g.append("path").datum(this.pr_diagnostics).attr("class", "line").attr("d", pr_line_position).attr("stroke", "url(#linear-gradient)").attr("fill", "none");
    ...
    this.roc_pr_g
      ...
      .on("mousemove", () => {
        ...
        this.tooltip
          ...
          .style("top", d3_test.event.pageY - 25 + "px")
          .html("<p> Area Under the Curve = " + d2.auc + "</p><br> \
              <p> Recall = " + d2.recall + "</p><br> \
              <p> Precision = " + d2.precision + "</p><br> \
              <p> Cutoff = " + d2.cutoff + "</p>");
  ...
  private init_var_imp_svg() {
    ...
    // Create new plot
    d3.select("#variable_importance_div").append("svg").attr("id", "variable_importance_plot").attr("width", box_width).attr("height", box_height);
  ...
  private draw_var_imp_axis() {
    ...
    this.var_imp_g.selectAll("text").style("text-anchor", "end").attr("dx", "-.8em").attr("dy", ".15em").attr("transform", "rotate(-65)");
  ...
  private init_scatter_axis() {
    ...
    this.scatter_y.domain([0, Math.max(d3_test.max(this.response.map((d) => d.expected)), d3_test.max(this.response.map((d) => d.observed)))]);
  }
  ...
  ngOnInit() {
    ...
    if (token) {
      ...
      this.cohortsService.getByUsername(this.tokenDecoded.username).subscribe((res: any[]) => {
