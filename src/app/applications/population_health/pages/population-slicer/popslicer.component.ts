...
import {
  ...
  FeatureCollection,
  PopulationPerson,
  MosaicColours, MosaicDomain, MosaicCode,
  RowChart, BarChart, PieChart, LeafletChoroplethChart, LeafletMarkerChart, PopulationManagementService
...
export class PopslicerComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.populationService.getCFServer().subscribe((res: any) => {
      ...
      if (this.totalsize === 0) {
        this.notificationService.info("There are no people in the dataset that you have access to view, you may need to request access to this data.");
      } else {
        this.buildCFServer();
        setTimeout(() => {
          this.createCharts();
  ...
  async refresh(queryFilter) {
    if (queryFilter !== this.lastQueryFilter) {
      ...
      await d3.json(this.origin.replace("phi", "population") + "/populations/getCrossfilter?filter=" + JSON.stringify(queryFilter), options).then((d) => {
  ...
  updateSummaries() {
    ...
    if (list.length > 0) {
      ...
      for (const e in list) {
        const chart = list[e];
        if (chart.anchorName() === "dc-data-count" || chart.anchorName() === "TotalPopulation" || chart.anchorName() === "CustBaseline" || chart.anchorName() === "TotalSelected" || chart.anchorName() === "PercentSelected") {
  ...
  updateCosts() {
    ...
    if (list.length > 0) {
      ...
      for (const e in list) {
        const chart = list[e];
        if (chart.anchorName() === "TotalCost" || chart.anchorName() === "CustomCost" || chart.anchorName() === "SelectedCost" || chart.anchorName() === "PercentCost") {
          this.myDC.chartRegistry.deregister(chart);
        }
      }
    }
    ...
    this.PercentCost.dimension(this.ndx)
      ...
      .formatNumber(() => {
        ...
        if (this.customcost !== "-") {
          custompercentage = Math.floor((parseInt(this.sumValues(this.filteredData["CstDimension"].values)) / parseInt(this.customcost)) * 100) + "%";
        }
        const selectedpercentage = (parseInt(this.sumValues(this.filteredData["CstDimension"].values)) / parseInt(this.totalcost)) * 100;
  ...
  updatePatientsCount() {
    this.patientsCount
      ...
      .formatNumber(() => {
        ...
        return "Selected <strong>" + percent + "%</strong> (<strong>" + selectedCount + "</strong> out of <strong>" + total + "</strong> records)";
      });
  }
  /* #endregion */
  ...
  createGPMap(dimension: any, group: any) {
    this.gpmapChartDetails = {
      ...
      popup: (d, feature) => {
        const GP = this.GPs.features.filter((x) => x.properties.Code === d.key);
        if (GP.length > 0) {
          return '<h5 class="ttipmap">' + GP[0].properties.Name + "</h5>" + '<p class="ttipmap">Total Population: ' + this.numberWithCommas(d.value) + "</p>";
        } else {
          return '<h5 class="ttipmap">' + d.key + '</h5><p class="ttipmap">Total Population: ' + this.numberWithCommas(d.value) + "</p>";
        }
      },
    };
