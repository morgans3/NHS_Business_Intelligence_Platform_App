...
export class RecentreferralsComponent implements OnInit {
  ...
  ccgoptions = [];
  gpoptions = [];
  displayedColumns: string[] = ["fullname", "nhs_number", "age_in_years", "specimen_date", "recommendation", "actions", "updated_date", "flags", "functions"];
  ...
  exportReferralsToCsv(hasMFA: boolean) {
    ...
    const options = {
      ...
      showTitle: false,
      useBom: true,
      headers: ["NHS Number", "Forename", "Surname", "Age", "Phone number", "Email address", "Date referred to virtual ward", "Specimen date"],
    };
  ...
  undoAction(item: VirtualWardPatient) {
    this.apiService.updateVWStatus(item.id, "Pending").subscribe((data: any) => {
      if (data && data.success) {
        this.notificationService.success("Status has been set back to Pending, this positive case is now available to review in VW Management Console.");
