...
export class ReportNotesComponent implements OnInit, OnChanges {
    ...
    getUserPhoto(username: string) {
        if (username) {
            const checkcurrentcollection = this.userPhotos.filter((x) => x.username === username);
            if (checkcurrentcollection && checkcurrentcollection.length > 0) {
                ...
                this.apiService.getUserProfileByUsername(username).subscribe((user: iUserDetails) => {
    ...
    removeMessage(message: iNotifications) {
        ...
        if (index) {
            ...
            this.notificationService.warning("Note no longer exists, unable to remove. Please refresh page for latest notes");
        }
    }
}
