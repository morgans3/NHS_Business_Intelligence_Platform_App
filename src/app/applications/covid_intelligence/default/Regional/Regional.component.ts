...
export class RegionalComponent implements OnInit {
  ...
  async refresh(queryFilter) {
    if (queryFilter !== this.lastQueryFilter) {
      ...
      await d3.json(this.origin.replace("cvi", "mosaic") + "/people/getCrossfilter?filter=" + JSON.stringify(queryFilter), options).then((d) => {
  ...
  createCharts() {
    ...
    this.suppliesSelect.dimension(this.can_you_get_essential_supplies_delivered).group(this.can_you_get_essential_supplies_deliveredGroup).controlsUseVisibility(true);
