...
export const DeprivationColors = ["#000000", "#40004b", "#762a83", "#9970ab", "#c2a5cf", "#e7d4e8", "#d9f0d3", "#a6dba0", "#5aae61", "#1b7837", "#00441b"];
...
export class OutbreaksComponent implements OnInit {
  ...
  displayedColumns: string[] = ["properties.area", "properties.crude_rate_per_thousand", "properties.crude_rate", "properties.positives", "properties.care_home_ratio", "properties.rate_diff"];
  ...
  runScanStats(initialRun: boolean, api_method: string, geo_level: string) {
    ...
    if (this.formatDate(this.date_start) === "2020-03-01" && this.formatDate(this.date_end) === this.formatDate(new Date()) && api_method === "cloak") {
      this.notificationService.warning("Please Select A Date Range Before Running Models");
      return;
    }
    let http_request = "";
    ...
    if (api_method === "rate") {
      switch (geo_level) {
        case "lsoa":
          ...
          this.displayedColumns = ["properties.area", "properties.la", "properties.crude_rate_per_thousand", "properties.crude_rate", "properties.positives", "properties.care_home_ratio", "properties.rate_diff"];
          break;
        case "msoa":
        ...
        case "la":
          ...
          this.displayedColumns = ["properties.area", "properties.crude_rate_per_thousand", "properties.crude_rate", "properties.positives", "properties.care_home_ratio", "properties.rate_diff"];
          break;
        case "utla":
    ...
    http_request = "https://smtp.nexusintelligencenw.nhs.uk/" + (api_method === "rate" ? "covid_crude_rate?" : "cloak?") + "date_start=" + this.formatDate(this.date_start) + "&" + "date_end=" + this.formatDate(this.date_end) + "&" + "geo_level=" + geo_level;
          ...
          (res: any) => {
            ...
            if (api_method === "rate") {
              ...
              this.dataSource.data.forEach((d) => {
                ...
                d.properties.previous_crude_rate_per_thousand = Math.round(100 * d.properties.previous_crude_rate_per_thousand) / 100;
              });
  ...
  plotScanStats(api_method: string) {
    ...
    if (api_method === "rate") {
      document.getElementById("polyRateContainer").innerHTML = '<div class="mapRatePoly"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></div>';
    } else {
      document.getElementById("polyCloakContainer").innerHTML = '<div class="mapCloakPoly"><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br><br></div>';
    }
    ...
    this.mapPoly = leafletChoroplethChart(api_method == "rate" ? "#polyRateContainer .mapRatePoly" : "#polyCloakContainer .mapCloakPoly", this.groupname)
      .dimension(area)
      .group(areaGroup)
      .width(width)
      .height(500)
      ...
      .featureStyle(this.showRateChange ? this.styleRateDiff : this.polyRateLSOA || this.polyRateMSOA || this.polyRateWard ? this.styleRateNoBoundary : this.polyCloak ? this.styleCloak : this.styleRate)
  ...
  rateKV(kv) {
    if (kv.value[0] || kv.value[0] == 0) {
      return "<b>Area</b> : " + kv.key + ", <br> <b>COVID Cases per 100 000 per week</b> : " + kv.value[0] + " (CI " + kv.value[1] + " - " + kv.value[2] + ")" + ", <br> <b>Positives Cases</b> : " + kv.value[3] + ", <br> <b>Proportion of Cases Within Care Homes</b> : " + kv.value[7] + "%, <br> <b>Date Range</b> : " + kv.value[4] + " <b>-</b> " + kv.value[5] + ", <br> <b>COVID Rate (per 100 000 per week) Change</b> : " + kv.value[6];
    }
  }
  ...
  plotIsochrones() {
    ...
    this.mapPoly = leafletChoroplethChart("#isochroneContainer .mapIsochrone", this.groupname)
      ...
      .popup((kv) => {
        return "<b>Isochrone Infection Density</b> : " + kv.value[1] + "<br> <b> Isochrone Travel-Time : " + kv.value[0] + " [mins walking]";
      });
  ...
  updateColourBar() {
    ...
    let halfOpacityPoint = Math.asin(0.5 * (Math.sin(1 + this.polyMinOpacity / 1000) + Math.sin(this.polyMinOpacity / 1000))) - this.polyMinOpacity / 1000;
    if (!this.polyCloak) {
    ...
    switch (true) {
      ...
      case this.showRateChange:
        colourBarUnits = " ";
        colourBarTitle = "Change in COVID Cases (Per 100 000 Per Week):" + " (" + this.formatDate(this.date_start) + " to " + this.formatDate(this.date_end) + ") " + "vs (" + this.formatDate(this.addDays(this.date_start, -this.n_weeks * 7)) + " to " + this.formatDate(this.addDays(this.date_end, -this.n_weeks * 7)) + ")";
        break;
      default:
        colourBarUnits = " ";
    ...
    if (this.showRateChange) {
      colourBar.innerHTML += '<div style="width:33%;float:left;text-align:left">' + -Math.round(100 * this.maxDiff) / 100 + "</div>" + '<div style="width:33%;float:left;text-align:center;">0</div>' + '<div style="width:33%;float:right;text-align:right;">' + Math.round(100 * this.maxDiff) / 100 + "</div>";
    } else {
      colourBar.innerHTML += '<div style="width:33%;float:left;text-align:left"> 0' + colourBarUnits + "</div>" + '<div style="width:33%;float:left;text-align:center;">' + Math.round(100 * halfOpacityPoint) / 100 + colourBarUnits + "</div>" + '<div style="width:33%;float:right;text-align:right;">' + Math.round(100 * maxOpacityPoint) / 100 + colourBarUnits + " - " + Math.round(100 * this.maxScore) / 100 + colourBarUnits + "</div>";
    }
  ...
  async refresh(queryFilter) {
    if (queryFilter !== this.lastQueryFilter) {
      ...
      await d3.json(this.origin.replace("cvi", "results") + "/tpindex/getCrossfilter?filter=" + JSON.stringify(queryFilter), options).then((d: any) => {
  ...
  downloadData() {
    ...
    if (this.polyRateLSOA) {
      ...
      options = {
        ...
        showTitle: true,
        useBom: true,
        headers: ["Area", "Local Authority", "Crude Rate (per 100 000)", "7-day Average Crude Rate (per 100 000)", "Lower 95% CI [7-day Average Crude Rate (per 100 000)]", "Upper 95% CI [7-day Average Crude Rate (per 100 000)]", "Number of Positives", "Number of Positives Within Care Homes", "Proportions of Positives Within Care Homes [%]", "Number of Weeks", "Start Date [yyyy-mm-dd]", "End Date [yyyy-mm-dd]", "7-day Average Crude Rate Difference", "Previous 7-day Average Crude Rate (per 100 000)", "Previous Number of Positives", "Previous Start Date [yyyy-mm-dd]", "Previous End Date [yyyy-mm-dd]"],
      };
      ...
      options = {
        ...
        showTitle: true,
        useBom: true,
        headers: ["Area", "MSOA Code", "Local Authority", "Crude Rate (per 100 000)", "7-day Average Crude Rate (per 100 000)", "Lower 95% CI [7-day Average Crude Rate (per 100 000)]", "Upper 95% CI [7-day Average Crude Rate (per 100 000)]", "Number of Positives", "Number of Positives Within Care Homes", "Proportions of Positives Within Care Homes [%]", "Number of Weeks", "Start Date [yyyy-mm-dd]", "End Date [yyyy-mm-dd]", "7-day Average Crude Rate Difference", "Previous 7-day Average Crude Rate (per 100 000)", "Previous Number of Positives", "Previous Start Date [yyyy-mm-dd]", "Previous End Date [yyyy-mm-dd]"],
      };
      ...
      options = {
        ...
        showTitle: true,
        useBom: true,
        headers: ["Electoral Ward", "Ward Code", "Local Authority", "Crude Rate (per 100 000)", "7-day Average Crude Rate (per 100 000)", "Lower 95% CI [7-day Average Crude Rate (per 100 000)]", "Upper 95% CI [7-day Average Crude Rate (per 100 000)]", "Number of Positives", "Number of Positives Within Care Homes", "Proportions of Positives Within Care Homes [%]", "Number of Weeks", "Start Date [yyyy-mm-dd]", "End Date [yyyy-mm-dd]", "7-day Average Crude Rate Difference", "Previous 7-day Average Crude Rate (per 100 000)", "Previous Number of Positives", "Previous Start Date [yyyy-mm-dd]", "Previous End Date [yyyy-mm-dd]"],
      };
    } else {
      data.forEach((d) => {
        this.csvData.push({
      ...
      options = {
        ...
        showTitle: true,
        useBom: true,
        headers: ["Area", "Crude Rate (per 100 000)", "7-day Average Crude Rate (per 100 000)", "Lower 95% CI [7-day Average Crude Rate (per 100 000)]", "Upper 95% CI [7-day Average Crude Rate (per 100 000)]", "Number of Positives", "Number of Positives Within Care Homes", "Proportions of Positives Within Care Homes [%]", "Number of Weeks", "Start Date [yyyy-mm-dd]", "End Date [yyyy-mm-dd]", "7-day Average Crude Rate Difference", "Previous 7-day Average Crude Rate (per 100 000)", "Previous Number of Positives", "Previous Start Date [yyyy-mm-dd]", "Previous End Date [yyyy-mm-dd]"],
      };
    }
